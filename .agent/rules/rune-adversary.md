...
If any auth, crypto, or payment logic is in the plan: MUST call `the rune-sentinel rule` for deep analysis.
...
If bottleneck patterns detected: call `the rune-perf rule` for quantitative analysis.
...
- Use `the rune-scout rule` to find all files the plan will modify or depend on
