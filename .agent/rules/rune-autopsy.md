...
Call `the rune-scout rule` with a request for a full project map. Ask scout to return:
...
Call `the rune-journal rule` to record that autopsy ran, the overall health score, and the surgery queue.
...
- Recommended next step: call `the rune-safeguard rule` on the top-priority module
...
### Next Step
Run the rune-safeguard rule on [top-priority module] before any refactoring.
```
