...
If the domain is unfamiliar or data is needed, invoke `the rune-research rule` before generating options. For product/market context, invoke `the rune-trend-scout rule`.
...
- Invoke `the rune-research rule` to search for how others solved similar problems (repos, articles, workarounds)
...
For approaches with many interacting variables, invoke `the rune-sequential-thinking rule` to reason through trade-offs systematically.
...
Pass the recommended approach back to `the rune-plan rule` for structuring into an executable implementation plan. Include:
...
### Next Step
Proceeding to the rune-plan rule with Option A. Constraints to honor: [list].
```
