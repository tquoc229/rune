...
Code quality analysis. Review finds bugs, bad patterns, security issues, and untested code. It does NOT fix anything — it reports findings and delegates: bugs go to the rune-fix rule, untested code goes to the rune-test rule, security-critical code goes to the rune-sentinel rule.
...
- If context is unclear: use `the rune-scout rule` to identify all files touched by the change
...
- If any security-sensitive code found (auth logic, input handling, crypto, payment): call `the rune-sentinel rule` for deep scan
...
- If untested code found: call `the rune-test rule` with specific instructions on what to test
...
After reporting:
- If any CRITICAL findings: call `the rune-fix rule` immediately with the finding details
- If any HIGH findings: call `the rune-fix rule` with the finding details
- If untested code: call `the rune-test rule` with specific coverage gaps identified
...
7. MUST flag untested code paths and recommend tests via the rune-test rule
...
| Security finding without sentinel escalation | HIGH | Any auth/crypto/payment code touched → MUST call the rune-sentinel rule |
