...
Call `the rune-scout rule` targeting the specific module. Ask scout to return:
...
Call `the rune-verification rule` and explicitly pass the characterization test file path.
...
### Next Step
Safe to proceed with: `the rune-surgeon rule` targeting [module]
```
