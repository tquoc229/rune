...
**SUB-SKILL**: Use `the rune-sentinel-env rule`
...
**REQUIRED SUB-SKILLS**: Use `the rune-scout rule`. For non-trivial tasks, use `the rune-ba rule`.
...
3. **BA gate** — determine if Business Analyst elicitation is needed:
   - If task is a Feature Request, Integration, or Greenfield → invoke `the rune-ba rule` for requirement elicitation
   - If task description is > 50 words or contains business terms (users, revenue, workflow, integration) → invoke `the rune-ba rule`
...
2. If ≥1 pack matches:
   - Read the file to load the matching PACK.md
...
**REQUIRED SUB-SKILL**: Use `the rune-plan rule`
...
4. If multiple valid approaches exist → invoke `the rune-brainstorm rule` for trade-off analysis
...
**REQUIRED SUB-SKILL**: Use `the rune-adversary rule`
...
When `the rune-plan rule` produces a **master plan + phase files** (non-trivial tasks):
...
**REQUIRED SUB-SKILL**: Use `the rune-test rule`
...
**REQUIRED SUB-SKILL**: Use `the rune-fix rule`
...
4. If stuck on unexpected errors → invoke `the rune-debug rule` (max 3 debug↔fix loops)
5. **Re-plan check** — before proceeding to Phase 5, evaluate:
   ...
   - If any trigger fires: invoke `the rune-plan rule` with delta context:
     ```
...
6. **Approach Pivot Gate** — if re-plan ALSO fails (implementation still blocked after revised plan):
   ...
   **Action**:
   ```
   Invoke the rune-brainstorm rule with:
     mode: "rescue"
...
**REQUIRED SUB-SKILL**: Use `the rune-preflight rule`
...
### 5b. Security
**REQUIRED SUB-SKILL**: Use `the rune-sentinel rule`
...
### 5c. Code Review
**REQUIRED SUB-SKILL**: Use `the rune-review rule`
- Pattern compliance: Follows project conventions
- Code quality: Clean, readable, maintainable
- Performance: No obvious bottlenecks

### 5d. Completion Gate
**REQUIRED SUB-SKILL**: Use `the rune-completion-gate rule`
...
Trigger: Invoke the rune-session-bridge rule at each boundary.
...
**REQUIRED SUB-SKILL**: Use `the rune-verification rule`
...
3. Use `the rune-hallucination-guard rule` to verify:
...
**RECOMMENDED SUB-SKILL**: Use `the rune-git rule` for semantic commit generation.
...
**REQUIRED SUB-SKILL**: Use `the rune-session-bridge rule`
...
| 2 PLAN | Task too complex | Break into smaller tasks, consider `the rune-team rule` |
| 3 TEST | Can't write tests (no test framework) | Skip TDD, write tests after implementation |
| 4 IMPLEMENT | Fix hits repeated bugs | `the rune-debug rule` (max 3 loops) → re-plan → if still blocked → **Approach Pivot Gate** → `rune:brainstorm(rescue)` |
...
| Scout Gate | scout output (files examined, patterns found) before Phase 2 | Invoke the rune-scout rule first |
