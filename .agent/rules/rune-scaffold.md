...
Invoke `the rune-ba rule` with the user's project description.
...
Invoke `the rune-research rule` to find:
...
Invoke `the rune-plan rule` with the Requirements Document from Phase 1 and research from Phase 2.
...
- Invoke `the rune-design rule` to generate design system
...
**Parallelization**: If plan has 3+ independent modules → invoke `the rune-team rule` to implement in parallel using worktrees.
...
Invoke `the rune-test rule` to generate tests based on acceptance criteria from Phase 1:
...
Invoke `the rune-verification rule` to run ALL checks:
...
5. **Security**: `the rune-sentinel rule` quick scan — no critical issues

<HARD-GATE>
If ANY check fails → fix the issue (invoke the rune-fix rule) and re-verify.
