...
Call `the rune-scout rule` for a full project map. Then use read the file on:
...
Call `the rune-journal rule` to record: audit date, overall health score, verdict, and CRITICAL count.
...
| Security Gate | sentinel report received before assembling final report | Invoke the rune-sentinel rule — do not skip |
| Deps Gate | dependency-doctor report received before assembling final report | Invoke the rune-dependency-doctor rule — do not skip |
