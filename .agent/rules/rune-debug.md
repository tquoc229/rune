...
Root cause analysis ONLY. Debug investigates — it does NOT fix. It traces errors through code, analyzes stack traces, forms and tests hypotheses, and identifies the exact cause before handing off to the rune-fix rule.
...
- Escalate to `the rune-problem-solver rule` for structured 5-Whys or Fishbone analysis
- Or escalate to `the rune-sequential-thinking rule` for multi-variable analysis
...
- Use `the rune-browser-pilot rule` if the issue is UI-related (console errors, network failures, visual bugs)
- Use `the rune-scout rule` to trace imports and identify all modules touched by the affected code path
...
- Use `the rune-browser-pilot rule` for UI hypotheses (inspect DOM, network, console)
...
3. **Route based on classification**:
   ...
   - Module needs redesign → Escalate to `the rune-plan rule` for redesign of the affected module
...
Produce structured output and hand off to the rune-fix rule.

- Write the Debug Report (see Output Format below)
- Call `the rune-fix rule` with the full report if fix is needed
...
[If attempt 3: "ESCALATION: 3-fix rule triggered. Recommending redesign via the rune-plan rule."]
...
| Exceeding 3 hypothesis cycles without escalation | MEDIUM | After 3 cycles: escalate to the rune-problem-solver rule or the rune-sequential-thinking rule |
...
- No code changes made — the rune-fix rule called with the report if fix is needed
