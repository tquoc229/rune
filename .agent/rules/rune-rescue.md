...
```
REQUIRED SUB-SKILL: the rune-autopsy rule
...
Check: does CLAUDE.md exist in project root?
  If NO:
    REQUIRED SUB-SKILL: the rune-onboard rule
...
```
REQUIRED SUB-SKILL: the rune-dependency-doctor rule
...
```
REQUIRED SUB-SKILL: the rune-journal rule
...
REQUIRED SUB-SKILL: the rune-session-bridge rule
...
```
REQUIRED SUB-SKILL: the rune-safeguard rule
...
```
REQUIRED SUB-SKILL: the rune-surgeon rule
→ Invoke `surgeon` with:
...
```
REQUIRED SUB-SKILL: the rune-review rule
...
|------|----------|------------|
| Autopsy Gate | autopsy report with health score before planning | Run the rune-autopsy rule first |
| Safety Gate | safeguard characterization tests passing before surgery | Run the rune-safeguard rule first |
