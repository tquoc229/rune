...
Block deploy if ANY of:
  ...
  [ ] Build fails
  [ ] sentinel CRITICAL issues detected (invoke the rune-sentinel rule if not already run)
...
```
REQUIRED SUB-SKILL: the rune-browser-pilot rule
...
```
REQUIRED SUB-SKILL: the rune-watchdog rule
...
```
REQUIRED SUB-SKILL: the rune-marketing rule
...
If user requested video content:
  REQUIRED SUB-SKILL: the rune-video-creator rule
...
|------|----------|------------|
| Test Gate | verification output showing all green | Run the rune-verification rule first |
| Security Gate | sentinel output with no CRITICAL findings | Run the rune-sentinel rule first |
