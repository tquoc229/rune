...
Output is a Requirements Document → hand off to the rune-plan rule for implementation planning.
</HARD-GATE>
...
If existing codebase → invoke `the rune-scout rule` for context before proceeding.
...
## Next Step
→ Hand off to the rune-plan rule for implementation planning
```
